...
function getMissWarning(shots, club, wind = 0, windDir = "N") {
  const list = shots.filter((shot) => shot.club === club && shot.result);
  if (list.length < 5) return null;
  const dangerous = ["Slice", "Hook", "Lost", "Penalty"];
  const dangerCount = list.filter((shot) => dangerous.includes(shot.result) || dangerous.includes(shot.shotShape)).length;
...
function calcHandicapIndex(diffs) {
  ...
  const sorted = [...diffs].sort((aa, bb) => aa - bb);
...
function exportCSV(shots) {
  ...
  const rows = shots.map((shot) => [fmtDate(shot.finishedAt || shot.startedAt), shot.hole, shot.club, shot.distance, shot.shotShape, shot.lie, shot.result, (shot.note || "").replace(/"/g, "'")]);
...
function DispersionMap({ shots, club }) {
  const filtered = shots.filter((shot) => (!club || shot.club === club) && shot.start && shot.end);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const counts = {};
    allShots.forEach((shot) => { if (shot.club) counts[shot.club] = (counts[shot.club] || 0) + 1; });
    const top = Object.entries(counts).sort((aa, bb) => bb[1] - aa[1])[0];
  ...
  const clubAvg = useMemo(() => {
    const m = {};
    CLUBS.forEach((name) => {
      const list = allShots.filter((shot) => shot.club === name && shot.distance > 0);
  ...
  const clubPersonal = useMemo(() => {
    const m = {};
    CLUBS.forEach((name) => { m[name] = allShots.filter((shot) => shot.club === name && shot.distance > 0).length >= MIN_SHOTS; });
    return m;
  }, [allShots]);
  ...
  // Session stats
  const longest = useMemo(() => shots.reduce((best, shot) => shot.distance > (best?.distance || 0) ? shot : best, null), [shots]);
  const avgDist = useMemo(() => shots.length ? Math.round(shots.reduce((sum, x) => s + (x.distance || 0), 0) / shots.length) : 0, [shots]);
  const goodRate = useMemo(() => shots.length ? Math.round(shots.filter((shot) => shot.result === "Good").length / shots.length * 100) : 0, [shots]);

  // Club stats this round
  const clubStats = useMemo(() => CLUBS.map((name) => {
    const list = shots.filter((shot) => shot.club === name && shot.distance > 0);
    if (!list.length) return null;
    return { name, count: list.length, avg: Math.round(list.reduce((acc, x) => acc + x.distance, 0) / list.length), best: Math.max(...list.map((shot) => shot.distance)) };
  ...
  const gapData = useMemo(() => {
    ...
    }).filter(Boolean).sort((aa, bb) => bb.avg - aa.avg);
    const gaps = [];
  ...
  const misses = useMemo(() => CLUBS.map((name) => {
    const list = allShots.filter((shot) => shot.club === name);
    if (list.length < 5) return null;
    const counts = { Left: 0, Right: 0, Short: 0, Long: 0 };
    list.forEach((shot) => { if (counts[shot.result] !== undefined) counts[shot.result]++; });
    const sorted = Object.entries(counts).sort((aa, b) => b[1] - a[1]);
    const [dir, cnt] = sorted[0];
    if (cnt / list.length < 0.25) return null;
    return { name, dir, pct: Math.round(cnt / list.length * 100), goodPct: Math.round(list.filter((shot) => shot.result === "Good").length / list.length * 100) };
  ...
  const visShots = useMemo(() => mapMode === "hole" ? shots.filter((shot) => shot.hole === hole) : shots, [shots, hole, mapMode]);
  ...
  function endRound() {
    const done = shots.filter((shot) => shot.distance > 0);
    if (done.length > 0) setAllShots((prev) => [...prev, ...done]);
    if (totals.played > 0 || done.length > 0) {
      const best = done.reduce((best, shot) => shot.distance > (best?.distance || 0) ? shot : best, null);
      setHistory((prev) => [{
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {shots.map((shot) => (
                        <div key={s.id} style={{ background: "#f9fafb", borderRadius: 12, padding: "10px 12px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 900, color: "#111827" }}>{shot.club} — {shot.distance} yards</div>
                            <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>Hole {shot.hole} · {shot.shotShape} · {shot.lie} · {shot.result} · {fmtTime(shot.finishedAt)}</div>
                            {shot.note && <div style={{ fontSize: 11, color: "#6b7280", marginTop: 2 }}>Note: {shot.note}</div>}
                          </div>
                          <button onClick={() => openMaps(shot.end)} style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 10, padding: "6px 10px", fontSize: 13, color: AMBER, cursor: "pointer", flexShrink: 0 }}>➤</button>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <Card>
                  <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                      {CLUBS.filter((c) => allShots.some((shot) => shot.club === c)).map((c) => (
      ...
      {showAIDebrief && debriefRound && (
        <AIDebrief
          round={debriefRound}
          shots={allShots.filter((shot) => shot.finishedAt && new Date(shot.finishedAt) >= new Date(debriefRound.date) - 86400000)}
